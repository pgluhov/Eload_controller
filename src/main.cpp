...
//SemaphoreHandle_t transmit_uart_mutex;
...
void INIT_IO(){ // Для инициализации кнопок
...
void setup() {     
  ...
  QueueHandleKeyboard = xQueueCreate(QueueElementSizeBtn, sizeof(btn_message_t)); 
  ...
  QueueHandleUart = xQueueCreate(QueueElementSizeUart, sizeof(message_uart)); 
